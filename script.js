...
if (compare(prompts, replies, text)) { 
  ...
  }else if (text.match(/evict/gi)) {
    product = "Once the police and sheriffs got into the building, they broke into the tenants' rooms. Then they started breaking things up, stealing, taking what the manongs had, broke the toilets that way there were no toilet facilities, so the tenants could never return.";
    $('<div class="message loading new"><figure class="avatar"><img src=" ' + './images/df'+ (randomNum+1) +'.jpg" /></figure><span></span></div>').appendTo($('.mCSB_container'));
  updateScrollbar();
    setTimeout(function() {
    $('.message.loading').remove();
...
function randomNumber(max) { 
  return Math.floor(Math.random() * max);
    ...
    $('.message.loading').remove();
  /*  $.getJSON("https://mhr1235.github.io/gpt2_simple_data/chatbot3.json", function(data) {
